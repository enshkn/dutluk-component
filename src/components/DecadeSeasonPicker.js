import React from 'react';
import { Form } from 'react-bootstrap';

function DecadeSeasonPicker({ selectedDecade, selectedSeason, onChangeDecade, onChangeSeason }) {
  const decades = [];
  const currentYear = new Date().getFullYear();
  const startDecade = Math.floor(currentYear / 10) * 10; // Bulunduğumuz on yılın başlangıç yılı

  // Geçmişten şu andaki on yılın başlangıcına kadar olan on yılları listeye ekleme
  for (let decadeStart = startDecade; decadeStart >= 1900; decadeStart -= 10) {
    decades.push(decadeStart);
  }

  const seasons = ['Spring', 'Summer', 'Autumn', 'Winter'];

  return (
    <div>
      <Form.Select
        value={selectedDecade}
        onChange={(e) => {
          onChangeDecade(parseInt(e.target.value));
        }}
      >
        <option value="">Select Decade</option>
        {decades.map((decade) => (
          <option key={decade} value={decade}>
            {decade}-{decade + 9}
          </option>
        ))}
      </Form.Select>
      <Form.Select
        value={selectedSeason}
        onChange={(e) => {
          onChangeSeason(e.target.value);
        }}
      >
        <option value="">Select Season</option>
        {seasons.map((season) => (
          <option key={season} value={season}>
            {season}
          </option>
        ))}
      </Form.Select>
    </div>
  );
}

export default DecadeSeasonPicker;
